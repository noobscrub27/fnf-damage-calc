...
export function checkIntimidate(gen: Generation, source: Pokemon, target: Pokemon) {
  const blocked =
    target.hasAbility('Clear Body', 'White Smoke', 'Hyper Cutter', 'Full Metal Body', 'Inner Focus') ||
    // More abilities now block Intimidate in Gen 8+ (DaWoblefet, Cloudy Mistral)
    (gen.num >= 8 && target.hasAbility('Own Tempo', 'Oblivious', 'Scrappy')) ||
...
export function checkScreenBreakers(pokemon: Pokemon, move: Move, affectedSide: Side) {
...
export function getFinalDamage(
  ...
  if (statusReducesDamage) damageAmount = Math.floor(damageAmount / 2);
