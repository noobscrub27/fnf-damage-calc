...
function calculateSMSSSV(gen, attacker, defender, move, field) {
    ...
    if (move.named('Weather Ball')) {
        ...
        type = (0, items_1.getOrbType)(attacker.item);
    }
    ...
    var type1Effectiveness = (0, util_2.getMoveEffectiveness)(gen, move, defender.types[0], isGhostRevealed, isDarkRevealed, field.isGravity, isRingTarget, isBoneMaster);
    var type2Effectiveness = defender.types[1]
        ? (0, util_2.getMoveEffectiveness)(gen, move, defender.types[1], isGhostRevealed, isDarkRevealed, field.isGravity, isRingTarget, isBoneMaster)
        : 1;
    var typeEffectiveness = type1Effectiveness * type2Effectiveness;
    if (defender.teraType && defender.teraType !== 'Stellar') {
        typeEffectiveness = (0, util_2.getMoveEffectiveness)(gen, move, defender.teraType, isGhostRevealed, isDarkRevealed, field.isGravity, isRingTarget, isBoneMaster);
    }
    ...
    var weightBasedMove = move.named('Heat Crash', 'Heavy Slam', 'Low Kick', 'Grass Knot', 'Shadow Avil', 'Shadow Trip');
    ...
    var hitsPhysical = move.overrideDefensiveStat !== 'spd' &&
...
function calculateBasePowerSMSSSV(gen, attacker, defender, move, field, hasAteAbilityTypeChange, desc, hit) {
    ...
    switch (move.name) {
        ...
        case 'Shadow Desolation':
            basePower = Math.floor(move.bp * ((0, util_2.checkForLoweredStat)(gen, defender.boosts) ? 1.5 : 1));
            desc.moveBP = basePower;
            break;
        case 'Low Kick':
        case 'Grass Knot':
...
function calculateBPModsSMSSSV(gen, attacker, defender, move, field, desc, basePower, hasAteAbilityTypeChange, turnOrder) {
    ...
    if ((move.named('Facade', 'Shadow Rage') && attacker.hasStatus('brn', 'par', 'psn', 'tox', 'frz')) ||
        ...
        var type1Effectiveness = (0, util_2.getMoveEffectiveness)(gen, move, types[0], isGhostRevealed, field.isGravity, isRingTarget, isBoneMaster);
        var type2Effectiveness = types[1] ? (0, util_2.getMoveEffectiveness)(gen, move, types[1], isGhostRevealed, field.isGravity, isRingTarget, isBoneMaster) : 1;
    ...
    else if ((attacker.hasItem('Truth Splicer') &&
        attacker.named('Omniverum') &&
        move.hasType('Fire', 'Dragon'))) {
        bpMods.push(5324);
        desc.attackerItem = attacker.item;
    }
    else if ((attacker.hasItem('Ideals Splicer') &&
        attacker.named('Omniverum') &&
...
function calculateAtModsSMSSSV(gen, attacker, defender, move, field, desc) {
    ...
    if ((attacker.hasItem('Thick Club') &&
        ...
        (((attacker.hasItem('Choice Band') && move.category === 'Physical') ||
            ...
            (attacker.hasItem('Eviomight') && ((_a = gen.species.get((0, util_1.toID)(attacker.name))) === null || _a === void 0 ? void 0 : _a.nfe)))) {
        atMods.push(6144);
        desc.attackerItem = attacker.item;
    }
    return atMods;
...
function calculateDfModsSMSSSV(gen, attacker, defender, move, field, desc, isCritical, hitsPhysical) {
    ...
    else if (defender.hasAbility('Surging Mindforce') &&
